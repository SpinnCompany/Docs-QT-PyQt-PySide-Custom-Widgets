...
<Zoomable src={useBaseUrl('/img/showcase/headernav.gif')} alt="QCustomHeaderNav" />
...
<Zoomable src={useBaseUrl('/img/showcase/headernav-dark.gif')} alt="QCustomHeaderNav in dark theme" />
...
Colours come from the design tokens, so they follow the active theme. Roles used: `accent`, `on-surface`, `on-surface-muted`, `surface`, `surface-muted`.
