...
![QCustomQRGenerator](/img/showcase/qrgenerator.png)
...
| `border(*args, **kwargs)` | Border. |
| `boxSize(*args, **kwargs)` | Box size. |
| `cacheEnabled(*args, **kwargs)` | Cache enabled. |
| `clearCache()` | Clear the QR code cache. |
| `colorMask(*args, **kwargs)` | Color mask. |
| `copyToClipboard()` | Copy the QR code to clipboard. |
| `data(*args, **kwargs)` | Data. |
| `embedImage(*args, **kwargs)` | Embed image. |
| `embeddedImageIcon(*args, **kwargs)` | Embedded image icon. |
| `errorCorrection(*args, **kwargs)` | Error correction. |
| `fillColor(*args, **kwargs)` | Fill color. |
| `generateQRCode()` | Generate the QR code pixmap and set it to the QLabel. |
| `getQRCodePixmap()` | Get the current QR code as QPixmap. |
| `gradientEndColor(*args, **kwargs)` | Gradient end color. |
| `gradientStartColor(*args, **kwargs)` | Gradient start color. |
| `moduleDrawer(*args, **kwargs)` | Module drawer. |
| `refreshQRCode()` | Refresh QR code when theme changes, only if colors are from palette. |
| `resizeQR()` | Scale pixmap to fit while maintaining aspect ratio |
| `saveQRCode(file_path=None)` | Save the generated QR code to file. |
| `setEmbeddedImageFromFileDialog()` | Open a file dialog to set the embedded image. |
| `sizeRatio(*args, **kwargs)` | Size ratio. |
| `version(*args, **kwargs)` | Version. |
