...
`QCustomScatterChart` is included in the free package under the GPLv3, so an application you distribute with it must be GPLv3 as well. **Custom Widgets Pro** is the commercial licence that lets you use it in closed-source software.
